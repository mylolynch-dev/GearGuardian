/**
 * @file oled_font5x7.h
 * @brief Minimal 5×7 pixel ASCII bitmap font for SSD1306 rendering.
 *
 * Each character entry is 5 bytes representing columns.
 * Bit 0 of each byte is the topmost row; bit 6 is the bottom row.
 * Characters are indexed starting from ASCII 0x20 (space).
 *
 * TODO (Phase 2): Replace this stub with the full 5×7 font table.
 * A complete public-domain 5×7 font is available in many embedded
 * display libraries (e.g. Adafruit GFX font5x7).
 */
...
/* Pixel height per character (does not include descender space) */
#define FONT5X7_CHAR_HEIGHT  7

/* Total pixel height per character cell including 1-row spacing */
#define FONT5X7_CELL_HEIGHT  8

/* Pixel width per character cell including 1-column spacing */
#define FONT5X7_CELL_WIDTH   6

/* First printable ASCII character */
#define FONT5X7_FIRST_CHAR   0x20

/* Number of characters in the table (0x20..0x7E inclusive) */
#define FONT5X7_NUM_CHARS    95

/**
 * @brief Font bitmap table.
 *
 * font5x7[i] contains the 5-column bitmaps for ASCII character (i + 0x20).
 * font5x7[0] = space (0x20), font5x7[1] = '!' (0x21), etc.
 *
 * TODO (Phase 2): Fill in all 95 entries with actual bitmap data.
 * The stub below contains only space and a placeholder pattern.
 */
static const uint8_t font5x7[FONT5X7_NUM_CHARS][FONT5X7_CHAR_WIDTH] = {
